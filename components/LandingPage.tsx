...
const services = [
  ...
  {
    title: "ИИ-помощник под ваш бизнес",
...
const benefits = [
  ...
  ["🤖", "Первый ИИ-помощник", "Соберёте ИИ-помощника для маркетинга, SMM, аналитики или клиентов."],
...
const steps = [
  ...
  "Улучшаем ответы, проверяем результат и собираем инструкции для ИИ-помощника.",
...
const testimonials = [
  ...
  },
  {
    ...
    text: "Очень практично: без лишней теории и сложных терминов. Теперь ИИ помогает готовить предложения и анализировать заявки.",
  },
  {
    name: "Марина, онлайн-школа",
    text: "Понравилось, что мы работали с моим проектом. ИИ-помощник уже помогает писать тексты для рассылок.",
  },
];
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      {[...Array(8)].map((_, index) => (
        <motion.span
          aria-hidden="true"
          className="absolute h-2 w-2 rounded-full bg-fuchsia/30 blur-[1px]"
          key={index}
          animate={{
            y: [0, -34, 0],
            x: [0, index % 2 ? 18 : -18, 0],
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-milk text-graphite">
      <section className="relative isolate px-5 pb-20 pt-6 md:px-8 md:pb-28">
        <div className="absolute inset-0 -z-20 bg-[radial-gradient(circle_at_18%_8%,rgba(217,30,143,0.13),transparent_25%),radial-gradient(circle_at_82%_5%,rgba(232,227,238,0.74),transparent_28%),linear-gradient(135deg,#F7F1EA,#fffaf7_58%,#F7D7EA)]" />
        ...
        <nav className="mx-auto flex max-w-7xl items-center justify-between rounded-full border border-white/70 bg-white/45 px-5 py-3 shadow-card backdrop-blur-xl">
          <a href="#top" className="text-sm font-black uppercase tracking-[0.18em]"><span className="text-fuchsia">Нейро</span><span className="text-graphite">Практикум</span></a>
        ...
        <div id="top" className="mx-auto grid max-w-7xl items-center gap-12 pt-16 lg:grid-cols-[1.03fr_0.97fr] lg:pt-24">
          <motion.div initial="hidden" animate="visible" variants={stagger}>
            ...
            <motion.h1 variants={fadeUp} transition={{ duration: 0.7 }} className="max-w-4xl text-5xl font-black leading-[0.95] tracking-tight md:text-7xl xl:text-8xl">
              За 2 дня соберите своего <span className="fuchsia-text">ИИ-помощника</span> для бизнеса
            </motion.h1>
            <motion.p variants={fadeUp} className="mt-7 max-w-2xl text-lg leading-8 text-softGraphite md:text-xl">
              Практикум для предпринимателей и специалистов: разберём, где ИИ действительно помогает, настроим рабочие инструменты и соберём понятную систему под ваши задачи.
            </motion.p>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96, y: 24 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.18 }} className="relative mx-auto w-full max-w-[520px]">
            <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 7, repeat: Infinity }} className="rounded-[2rem] border border-white/80 bg-white/75 p-4 shadow-card backdrop-blur-xl">
              ...
              <div className="px-3 pb-3 pt-5">
                <p className="text-sm font-black uppercase tracking-[0.22em] text-fuchsia">Автор и тренер</p>
                <h3 className="mt-2 text-4xl font-black text-graphite">Наталья Руцкая</h3>
                <p className="mt-3 text-base leading-7 text-softGraphite/80">Маркетолог, помогаю бизнесу внедрять ИИ в контент, рекламу и ежедневные задачи</p>
                <div className="mt-5 grid grid-cols-3 gap-3 text-center">
                  {["Промпты", "Шаблоны", "Разбор"].map((item) => (
                    <div key={item} className="rounded-2xl bg-blush/70 p-3 text-sm font-bold text-darkFuchsia">{item}</div>
                  ))}
                </div>
              </div>
            </motion.div>
